...
botoes.forEach(botao => {
  botao.addEventListener("click", () => {
    ...
    if (botao.dataset.tab === "ranking" && (!usuarioAtual || !usuarioAtual.admin)) {
      return;
    }
...
async function carregarPerfil(idUsuario) {
  ...
  const btnRanking = document.querySelector('[data-tab="ranking"]');
  const abaRanking = document.getElementById("ranking");
  ...
  if (data.admin === true) {
    ...
    abaRanking?.classList.remove("hidden");
  } else {
    ...
    abaRanking?.classList.add("hidden");

    if (abaRanking?.classList.contains("active")) {
      abaRanking.classList.remove("active");
      document.getElementById("figurinhas")?.classList.add("active");

      botoes.forEach(botao => botao.classList.remove("active"));
      document.querySelector('[data-tab="figurinhas"]')?.classList.add("active");
    }
  }
...
async function atualizarRanking() {
  ...
  const rankingPodio = document.getElementById("rankingPodio");
  ...
  rankingPodio.innerHTML = ordemPodio.map((pessoa) => {
    const posicaoReal = ranking.findIndex(item => item.id === pessoa.id) + 1;
    const classe = `podium-card-position-${posicaoReal}`;

    return `
      <article class="podium-card ${classe}">
        <div class="podium-card-rank">${posicaoReal}º</div>
        <div class="podium-card-avatar">${pessoa.nome.charAt(0).toUpperCase()}</div>
        <div class="podium-card-copy">
          <strong>${pessoa.nome}</strong>
          <span>${pessoa.setor}</span>
        </div>
        <div class="podium-card-score">
          <strong>${pessoa.porcentagem}%</strong>
          <span>${pessoa.unicas}/${figurinhas.length} únicas</span>
        </div>
      </article>
    `;
  }).join("");
  ...
  rankingLista.innerHTML = ranking.map((pessoa, index) => {
    ...
    return `
      <article class="ranking-simple-row">
        <div class="ranking-simple-position">${posicao}º</div>

        <div class="ranking-simple-user">
          <strong>${pessoa.nome}</strong>
          <span>${pessoa.setor}</span>
        </div>

        <div class="ranking-simple-progress">
          <strong>${pessoa.porcentagem}%</strong>
          <span>completo</span>
        </div>

        <div class="ranking-simple-metrics">
          <div>
            <strong>${pessoa.unicas}/${figurinhas.length}</strong>
            <span>únicas</span>
          </div>
