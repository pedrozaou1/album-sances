...
async function atualizarRanking() {
  ...
  rankingLista.innerHTML = ranking.map((pessoa, index) => {
    ...
    return `
      <article class="ranking-row-modern">
        <div class="ranking-row-position">${posicao}</div>

        <div class="ranking-row-user">
          <div class="ranking-row-avatar">${pessoa.nome.charAt(0).toUpperCase()}</div>
          <div>
            <strong>${pessoa.nome}</strong>
            <span>${pessoa.setor}</span>
          </div>
        ...
        <div class="ranking-row-progress">
          <div class="ranking-row-progress-top">
            <span>Progresso do álbum</span>
            <strong>${pessoa.porcentagem}%</strong>
          </div>
          <div class="ranking-row-progress-bar">
            <div style="width:${pessoa.porcentagem}%"></div>
          </div>
        </div>

        <div class="ranking-row-metric">
          <strong>${pessoa.unicas}</strong>
          <span>únicas</span>
        </div>
