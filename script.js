...
async function atualizarRanking() {
  ...
  rankingLista.innerHTML = ranking.map((pessoa, index) => {
    ...
    return `
      <article class="ranking-line ${index < 3 ? `ranking-line-top ranking-line-top-${index + 1}` : ""}">
        <div class="ranking-line-position">${posicao}</div>

        <div class="ranking-line-person">
          <strong>${pessoa.nome}</strong>
          <span>${pessoa.setor}</span>
        </div>

        <div class="ranking-line-details">
          <span><b>${pessoa.unicas}/${figurinhas.length}</b> únicas</span>
          <i></i>
          <span><b>${pessoa.total}</b> figurinhas</span>
          <i></i>
          <span><b>${pessoa.repetidas}</b> repetidas</span>
          <i></i>
          <span><b>${pessoa.pacotes_abertos || 0}</b> pacotes</span>
        </div>

        <div class="ranking-line-percent">
