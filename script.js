...
async function atualizarRanking() {
  ...
  rankingLista.innerHTML=ranking.map((p,i)=>{const m=i===0?"🥇":i===1?"🥈":i===2?"🥉":`${i+1}º`;return `<div class="ranking-row ${i<3?`ranking-top-${i+1}`:""}"><div class="ranking-position">${m}</div><div class="ranking-person"><strong>${p.nome}</strong><span>${p.setor}</span></div><div class="ranking-progress-wrap"><div class="ranking-progress-label"><span>${p.unicas}/${figurinhas.length} únicas</span><strong>${p.porcentagem}%</strong></div><div class="ranking-progress-bar"><div style="width:${p.porcentagem}%"></div></div></div><div class="ranking-stats"><span><strong>${p.total}</strong> figurinhas</span><span><strong>${p.repetidas}</strong> repetidas</span><span><strong>${p.pacotes_abertos||0}</strong> pacotes</span></div></div>`;}).join("");
}
