...
async function atualizarRanking() {
  ...
  const ranking = usuarios.map(usuario => {
    ...
    const repetidas = itens.reduce((soma, item) => soma + Math.max(Number(item.quantidade || 0) - 1, 0), 0);
    const porcentagem = figurinhas.length ? Math.round((unicas / figurinhas.length) * 100) : 0;
    return { ...usuario, unicas, total, repetidas, porcentagem };
  }).sort((a,b) => b.unicas-a.unicas || b.total-a.total || Number(b.pacotes_abertos||0)-Number(a.pacotes_abertos||0));

  rankingLista.innerHTML = ranking.map((pessoa, index) => {
    const posicao = index === 0 ? "🥇" : index === 1 ? "🥈" : index === 2 ? "🥉" : `${index + 1}º`;
    return `
      <div class="ranking-card-clean ${index < 3 ? `ranking-podium-${index + 1}` : ""}">
        <div class="ranking-rank-clean">${posicao}</div>
        <div class="ranking-user-clean">
          <strong>${pessoa.nome}</strong>
          <span>${pessoa.setor}</span>
        </div>
        <div class="ranking-percent-clean" style="--percent:${pessoa.porcentagem * 3.6}deg">
          <div><strong>${pessoa.porcentagem}%</strong><small>${pessoa.unicas}/${figurinhas.length}</small></div>
        </div>
        <div class="ranking-numbers-clean">
          <span><strong>${pessoa.total}</strong><small>figurinhas</small></span>
          <span><strong>${pessoa.repetidas}</strong><small>repetidas</small></span>
          <span><strong>${pessoa.pacotes_abertos || 0}</strong><small>pacotes</small></span>
        </div>
      </div>`;
  }).join("");
}
