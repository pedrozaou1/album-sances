...
function criarPopupPacote() {
  ...
  popup.innerHTML = `
    <div class="pack-modal">
      <div class="pack-bg-grid"></div>
      <div class="sparkles"></div>
      <div class="pack-title">Abrindo pacote...</div>
...
function atualizarAlbum() {
  ...
  figurinhas.forEach(fig => {
    cards += albumUsuario[fig.id] ? `<div class="card album-card ${classeRaridade(fig)}"><div class="quantity-badge">${albumUsuario[fig.id].quantidade}</div><div class="photo">${imagemOuEmoji(fig,false)}</div><h3>${fig.nome}</h3><span>${fig.setor}</span><small class="raridade raridade-${fig.raridade === "normal" ? "incomum" : fig.raridade}">${textoRaridade(fig)}</small></div>` : criarCardFigurinha(fig,true);
  });
  albumSection.innerHTML = `<div class="album-hero"><div class="album-progress-ring" style="--album-progress:${porcentagem*3.6}deg"><div><strong>${porcentagem}%</strong><span>completo</span></div></div><div class="album-hero-copy"><span class="eyebrow">Sua coleção</span><h2>Meu Álbum</h2><p>Acompanhe seu progresso e todas as figurinhas conquistadas.</p></div></div><div class="album-metrics"><div class="album-metric"><span>Figurinhas únicas</span><strong>${conquistadas}<small>/${total}</small></strong></div><div class="album-metric"><span>Total na coleção</span><strong>${totalFigurinhas}</strong></div><div class="album-metric"><span>Repetidas</span><strong>${repetidas}</strong></div></div><div class="progress-box progress-box-premium"><div class="progress-label"><strong>Progresso geral</strong><span>${conquistadas} de ${total}</span></div><div class="progress-bar"><div class="progress" style="width:${porcentagem}%;"></div></div></div><div class="cards album-grid">${cards}</div>`;
}

async function atualizarRanking() {
  const rankingLista=document.getElementById("rankingLista");
  const {data:usuarios,error:eu}=await db.from("profiles").select("*");
  const {data:albuns,error:ea}=await db.from("album").select("*");
  if(eu||ea){rankingLista.innerHTML="<p>Erro ao carregar ranking.</p>";return;}
  const ranking=usuarios.map(u=>{const a=albuns.filter(i=>i.usuario_id===u.id);const unicas=a.length;const total=a.reduce((s,i)=>s+Number(i.quantidade||0),0);const repetidas=a.reduce((s,i)=>s+Math.max(Number(i.quantidade||0)-1,0),0);const porcentagem=figurinhas.length?Math.round(unicas/figurinhas.length*100):0;return{...u,unicas,total,repetidas,porcentagem};}).sort((a,b)=>b.unicas-a.unicas||b.total-a.total||Number(b.pacotes_abertos||0)-Number(a.pacotes_abertos||0));
  rankingLista.innerHTML=ranking.map((p,i)=>{const m=i===0?"🥇":i===1?"🥈":i===2?"🥉":`${i+1}º`;return `<div class="ranking-row ${i<3?`ranking-top-${i+1}`:""}"><div class="ranking-position">${m}</div><div class="ranking-person"><strong>${p.nome}</strong><span>${p.setor}</span></div><div class="ranking-collection"><div class="ranking-collection-top"><span>${p.unicas}/${figurinhas.length} únicas</span><strong>${p.porcentagem}%</strong></div><div class="ranking-progress-bar"><div style="width:${p.porcentagem}%"></div></div></div><div class="ranking-stats"><span><strong>${p.total}</strong><small>figurinhas</small></span><span><strong>${p.repetidas}</strong><small>repetidas</small></span><span><strong>${p.pacotes_abertos||0}</strong><small>pacotes</small></span></div></div>`;}).join("");
}
...
async function animarRoleta(vencedores){const wheel=document.getElementById("rouletteWheel"),nome=document.getElementById("rouletteName"),res=document.getElementById("resultadoSorteio"),nomes=participantesSorteio.map(i=>i.nome);wheel.classList.add("spinning");res.innerHTML="<span>Sorteando...</span>";await new Promise(resolve=>{const inicio=Date.now(),it=setInterval(()=>{nome.textContent=nomes[Math.floor(Math.random()*nomes.length)]||"Sorteando...";if(Date.now()-inicio>=3600){clearInterval(it);resolve();}},85)});wheel.classList.remove("spinning");nome.textContent="Resultado!";res.innerHTML=`<small>Funcionários sorteados</small><div class="winner-list">${vencedores.map((v,i)=>`<div class="winner-card"><span>${i+1}</span><strong>${v.nome}</strong><small>${v.setor}</small></div>`).join("")}</div>`;}
